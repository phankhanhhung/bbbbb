...
Step fields: `id`, `parent`, `edge_type ∈ {seq, case, contradiction, merge_ref}`, `case_label`, `narrative` (LaTeX + anchor markup), `anchors`, `scene`, `widget_state`, `transition_hints?`, `alt_text?`, `author_notes?`.
...
- **DAT-13 [P2] SHOULD** — `transition_hints` cho phép override animation mặc định: thứ tự xuất hiện, thời lượng, kiểu di chuyển (đường thẳng/cong).
